...
app.post('/api/habilidades', async (req, res) => {
    ...
    try {
        const [habilidad] = await db('habilidades').insert({ coste, tipo, descripcion, imagen }).returning('id');

        if (!campeon_nombre) {
            return res.status(400).json({ error: 'Falta el nombre del campeón' });
        }

        // habilidad.id es el número
        await db('campeon_habilidad').insert({ campeon_nombre, habilidad_id: habilidad.id });
